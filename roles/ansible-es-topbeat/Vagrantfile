...
Vagrant.configure(2) do |config|
  config.vm.define "topbeat" do |topbeat|
    topbeat.vm.box = "mrlesmithjr/trusty64"
    topbeat.vm.hostname = "topbeat"

    topbeat.vm.network :private_network, ip: "192.168.202.201"

    topbeat.vm.provider "virtualbox" do |vb|
     vb.memory = "512"
    end
    topbeat.vm.provision :shell, inline: 'ansible-playbook -i /vagrant/hosts -c local /vagrant/playbook.yml --limit "topbeat"'
  end
